...
def _is_publisher_fallback_journal(
    journal: str,
    publisher_name: str,
    known_journals: list[str] | None = None,
) -> bool:
    """저널 추론이 실패해 출판사명으로만 떨어진 경우를 판정한다."""
    # A publisher name can also be a real journal name, e.g. Science.
    if not journal:
        return False

    normalized_journal = journal.strip().lower()
    if known_journals:
        known = {name.strip().lower() for name in known_journals}
        if normalized_journal in known:
            return False

    return normalized_journal == publisher_name.strip().lower()
...
def run_pipeline(dry_run: bool = False) -> dict:
    ...
    for msg_id in msg_ids:
        try:
            ...
            # 각 paper의 누락 필드 보완. 저널 추론이 출판사명 fallback이면 비논문 메일로 보고 제외.
            today = __import__("datetime").date.today().isoformat()
            valid_papers = []
            for paper in papers:
                if not paper.journal:
                    inferred_journal = infer_journal(sender, subject, publishers)
                    if _is_publisher_fallback_journal(
                        inferred_journal,
                        pub_name,
                        publishers[pub_key].get("journals", []),
                    ):
                        logger.warning(
                            "저널 추론 실패로 저장 제외 (publisher=%s): %s",
                            pub_name,
                            paper.title[:80],
                        )
                        continue
                    paper.journal = inferred_journal
                if _is_excluded_journal(paper.journal):
                    logger.info(
                        "제외 저널 스킵 (journal=%s): %s",
                        paper.journal,
                        paper.title[:80],
                    )
                    continue
