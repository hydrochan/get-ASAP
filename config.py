...
# 방문자 지표용 salt (익명 방문자 IP 해싱).
# hash = sha256(VISIT_HASH_SALT + ip).hexdigest() — DB에는 raw IP가 아니라 이 해시만 저장됨.
# salt가 비밀이어야 DB 유출 시에도 IPv4 전수 대입으로 방문 IP를 역산하는 것을 막을 수 있으므로
# .env에 안정적인(재시작해도 동일한) 랜덤 문자열로 VISIT_HASH_SALT를 설정할 것.
# 미설정 시: 프로세스마다 임시 salt를 생성 → 앱은 정상 동작하나 재시작 시 total/누적 지표가 리셋됨.
import secrets as _secrets
VISIT_HASH_SALT = os.getenv("VISIT_HASH_SALT", "").strip()
VISIT_HASH_SALT_IS_EPHEMERAL = False
if not VISIT_HASH_SALT:
    VISIT_HASH_SALT = _secrets.token_hex(32)
    VISIT_HASH_SALT_IS_EPHEMERAL = True
